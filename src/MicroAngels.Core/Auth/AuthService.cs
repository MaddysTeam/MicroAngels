    ...
    public static class AuthService
    {
        ...
        public static async Task<AuthTokenResponse> RequestAccesstokenAsync(AuthTokenRequest request, AuthType authType)
        {
            ...
            if (authType == AuthType.byCredential)
                tokenResponse = await tokenClient.RequestClientCredentialsAsync(request.Api).ConfigureAwait(false);
            else
                tokenResponse = await tokenClient.RequestResourceOwnerPasswordAsync(request.UserName, request.Password, request.Api).ConfigureAwait(false);
